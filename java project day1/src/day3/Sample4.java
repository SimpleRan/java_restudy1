...
public class Sample4 {
	...
	public static void main(String[] args) {
		// 객체지향 프로그래밍이란?
		// 모두 계산기를 사용해 보았을 것이다. 계산기에 숫자 3을 입력하고 + 기호를 
		// 입력한 후 4를 입력하면 결괏값으로 7을 보여 준다. 
		...
		//add 메소드는 매개변수 num으로 받은 값을 이전에 계산한 결괏값에 더한 후 
		// 돌려주는 메소드이다. 이전에 계산한 결괏값을 유지하기 위해서 
		// result 전역 변수(static)를 사용했다.
		
	// 만일 Sample 클래스에서 2대의 계산기가 필요한 상황이 발생하면 어떻게 해야 할까? 
	// 각 계산기는 각각의 결괏값을 유지해야 하기 때문에 위와 같이 Calculator 클래스 
	// 하나만으로는 결괏값을 따로 유지할 수 없다.
	// 이런 상황을 해결하려면 다음과 같이 클래스를 각각 따로 만들어야 한다.
		...
		// 그때마다 클래스를 추가할 것인가? 여기에 빼기나 곱하기 등의 기능을 추가해야 
		// 한다면 상황은 점점 더 어려워질 것이다.
		...
		// 프로그램을 실행하면 클래스 2개를 사용했을 때와 동일한 결과가 출력
			
		//Calculator 클래스로 만든 별개의 계산기 cal1, cal2(이것을 객체라고 부른다)가 
		// 각각의 역할을 수행한다. 그리고 계산기(cal1, cal2)의 결괏값 역시 다른 계산기의 
		//결괏값과 상관없이 독립적인 값을 유지한다. 객체를 사용하면 계산기 대수가 늘어나더라도 
