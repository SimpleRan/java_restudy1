...
public class Sample2 {
	...
	public static void main(String[] args) {
		...
		// 우리가 믹서기에 넣는 과일은 입력이 되고 과일 쥬스는 그 출력(리턴값)이 된다. 그렇다면 믹서기는 무엇인가?
		...
		// 이 때가 바로 메소드가 필요한 때이다. 여러 번 반복해서 사용된다는 것은 언제고 또다시 사용할 만한 
		// 가치가 있는 부분이라는 뜻이다. 즉, 이러한 경우 이것을 한 뭉치로 묶어서 "어떤 입력값을 주었을 때 
		// 어떤 리턴값을 돌려준다"라는 식의 메소드를 작성하는 것이 현명한 일일 것이다.
		...
		// 메소드에 들어오는 입력값과 리턴값에 대해서 자세히 알아보도록 하자.
			...
			// 리턴자료형은 메소드 수ㅐㅎㅇ 후 돌려줄 값으 자료형을 의미한다. 메소드의 리턴값은 return이라는 명령을 사용한다
			...
			// 왜냐하면 sum 메소드의 리턴타입이 int로 되어 있기 때문이다. 
			// sum 메소드의 리턴타입은 int sum(int a, int b) { ... 에서 보듯이 메소드명(sum) 바로 좌측에 표기하도록 되어 있다.
			...
			// 3과 4의 합은 7입니다.
			// 아마도 여러분은 다음과 같은 질문을 할지도 모른다. "3과 4의 합은 7입니다." 라는 
			// 문장을 출력해 주었는데 왜 리턴값이 없다는 것인가? 이 부분이 초보자들이 혼동스러워 
			// 하는 부분이기도 한데 System.out.println 문은 메소드내에서 사용되어지는 문장일 뿐이다. 
		...
		// 특별한 경우에 메소드를 빠져나가기를 원할 때 return을 단독으로 사용하여 메소드를 즉시 빠져나갈 수 있다. 
			...
			// 이 메소드 역시 리턴값은 없다. 문자열을 출력한다는 것과 리턴값이 있다는 것은 전혀 다른 말이다. 
			// 혼동하지 말도록 하자. 메소드의 리턴값은 오로지 return문에 의해서만 생성된다. 
			// 이 메소드는 입력값으로 "fool" 이라는 값이 들어오면 문자열을 출력하지 않고 메소드를 즉시 빠져나간다.
			// 이렇게 메소드 수행 시 특정 조건에 따라 메소드를 즉시 빠져나가고 싶은 경우 return 문을 이용하면 된다. 
		...
		// 메소드 내에서 선언된 변수의 효력 범위
			...
			// 아래의 예를 보자.
				...
				// 당연히 varTest 메소드에서 a의 값을 1만큼 증가시켰으니 2가 출력되어야 할 것 같지만 프로그램을 
				// 실행시켜 보면 1이라는 결과 값이 나온다.
				...
				// 위에서 변수이름을 a로 사용한 varTest메소드는 다음처럼 변수이름을 b로 사용한 
				// varTest와 기능적으로 완전히 동일하다.
				...
				// 따라서 a = sample.varTest(a)처럼 하면 a의 값은 다시 varTest 메소드의 리턴값으로 대입된다. 
				...
				// 이전 예제에서는 a 라는 int 자료형 변수를 main 메서드에 선언했는데 위 예제에는 
				// Sample 클래스의 객체변수로 선언했다. 그리고 varTest 메소드는 Sample 클래스의 
				// 객체를 입력받아 해당 객체의 객체변수 a의 값을 1만큼 증가시키는 역할을 하도록 수정했다. 
				// 그리고 main메소드에서는 varTest메소드에 1이라는 값을 전달하던것을 Sample 클래스의 
				// 객체인 sample을 넘기도록 수정했다.

				// 이렇게 수정하고 프로그램을 실행시켜보면 sample객체의 객체변수 a의 값이 원래는 1이었는데 
				// varTest 메소드 실행 후 1만큼 증가되어 2가 출력되는 것을 확인할 수 있다.

				// 여기서 주목해야 하는 부분은 varTest 메소드의 입력 파라미터가 값이 아닌 
				// Sample 클래스의 객체라는데 있다. 이렇게 메소드가 객체를 전달 받으면 
				// 메소드 내의 객체는 전달받은 객체 그 자체로 수행된다. 
				// 따라서 입력으로 전달받은 sample 객체의 객체변수 a의 값이 증가하게 되는 것이다.

				// 메소드의 입력항목이 값인지 객체인지를 구별하는 기준은 입력항목의 자료형이 
				// primitive 자료형인지 아닌지에 따라 나뉜다. int 자료형과 같은 primitive 자료형인 경우 
				// 값이 전달되고 그 이외의 경우(reference 자료형)는 객체가 전달된다.
