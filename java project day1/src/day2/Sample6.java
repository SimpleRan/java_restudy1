...
public class Sample6 {
	...
	public static void main(String[] args) {
		...
		// while문의 기본 구조 - 다음은 while문의 기본 구조이다
		// while (조건문) {
	    // <수행할 문장1>;
	    // <수행할 문장2>;
	    // <수행할 문장3>;
		...
		//“열 번 찍어 안 넘어 가는 나무 없다” 라는 속담을 자바로 코딩하면 다음과 같이 될 것이다.
		 ...
		 // 그리고 나무를 treeHit번 만큼 찍었음을 알리는 문장을 출력하고 
		 // treeHit가 10이 되면 “나무 넘어갑니다”라는 문장을 출력하고 
		 // treeHit < 10라는 조건문이 거짓이 되어 while문을 빠져 나가게 된다.

		 // 여기서 treeHit++ 는 프로그래밍을 할 때 매우 자주 사용하는 기법으로
		 // treeHit의 값을 1만큼씩 증가시킬 목적으로 쓰이는 것이다
		
		 // 무한루프(Loop)
		 // 무한 루프는 무한히 반복한다는 의미이다. 
		 // 자바에서 무한루프는 while문으로 구현 할수 있다. 
		 // 우리가 사용하는 프로그램들 중에서 무한루프의 개념을 사용하지 않는 프로그램은 
		 ...
		 // 자판기가 제 역할을 하려면 커피의 양을 따로이 검사를 해서 커피가 다 떨어지면 
		 // while문을 멈추게 하고 "판매중지"란 문구를 자판기에 보여야 할 것이다. 
		 ...
		 // money가 300으로 고정되어 있으니까 while (money > 0)에서
		 // money는 0보다 크기 때문에 항상 참이다. 따라서 무한 루프를 돌게 된다. 
		 // 그리고 while문이 수행되면 coffee--에 의해서 coffee의 개수가 한 개씩 줄어들게 된다. 
		 // 만약 coffee가 0이 되면 if (coffee == 0) 문장이 참이 되므로 break가 호출되어 
		 // while문을 빠져 나간다.
		 ...
		 // a가 10보다 작은 동안 a는 1만큼씩 계속 증가한다. if (a % 2 == 0) 
		 // (2로 나누었을 때 나머지가 0인 경우)이 참이 되는 경우는 a가 짝수일 때이다.
		 // 즉, a가 짝수이면 continue 문장이 수행된다. 
		 // 이 continue문은 while문의 맨 처음(조건문: a<10)으로 돌아가게 하는 명령어이다. 
		 // 따라서 위의 예에서 a가 짝수이면 System.out.println(a)는 수행되지 않기 
		 // 때문에 홀수만 출력될 것이다
	}
